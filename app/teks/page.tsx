...
import { getTeks } from "lib/teks";

export default async function TeksListPage() {
	const teks = await getTeks();

	const filter = "All"; // Default filter, as useState/useEffect are removed

	const filteredTeks =
		filter === "All" ? teks : teks.filter((tek) => tek.difficulty === filter);
		...
		<div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
			...
			<div className="text-center mb-12">
				...
				<p className="text-lg text-[var(--color-forest-brown)] max-w-2xl mx-auto leading-relaxed">
					Step-by-step techniques I've tested and refined through countless
					growing cycles. From beginner-friendly setups to advanced cultivation
					methods.
				</p>
			</div>
			...
			<div className="flex flex-wrap justify-center gap-3 mb-12">
				{(["All", "Beginner", "Intermediate", "Advanced"] as const).map(
					(level) => (
						<button
							key={level}
							// onClick={() => setFilter(level)} // Removed onClick as state is removed
						...
						<div className="flex items-start">
							...
							<div>
								...
								<p className="text-sm text-[var(--color-forest-brown)]">
									All techniques focus on culinary mushroom cultivation. Always
									research local laws and practice proper sterile technique.
								</p>
							</div>
						</div>
					</div>
					...
					<div className="organic-card p-6 border-l-4 border-green-400">
						<div className="flex items-start">
							...
							<div>
								...
								<p className="text-sm text-[var(--color-forest-brown)]">
									Start with beginner techniques and gradually work your way up.
									Each tutorial builds on previous skills.
								</p>
							</div>
						</div>
					</div>
